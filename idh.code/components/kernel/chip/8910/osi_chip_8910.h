...
static inline uint64_t osiChipHwTick(void)
{
    for (;;)
    {
...
{
#if defined(CONFIG_KERNEL_PROFILE_CLOCK2M)
    return hwp_timer4->hwtimer_curval_l;
#elif defined(CONFIG_KERNEL_PROFILE_CLOCK32K)
    return hwp_idle->idl_32k_ref;
#endif
}

static inline uint32_t osiChipProfileTickFreq(void)
{
#if defined(CONFIG_KERNEL_PROFILE_CLOCK2M)
    return 2000000;
#elif defined(CONFIG_KERNEL_PROFILE_CLOCK32K)
