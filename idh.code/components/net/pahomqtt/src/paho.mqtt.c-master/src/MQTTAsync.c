...
//{
//	pthread_mutex_destroy((pthread_mutex_t *)(* mqttasync_mutex));
//	pthread_mutex_destroy((pthread_mutex_t *)(* mqttcommand_mutex));
//	pthread_mutex_destroy((pthread_mutex_t *)(* socket_mutex));
//	pthread_mutex_destroy((pthread_mutex_t *)(send_cond->mutex));
//	pthread_cond_destroy((pthread_cond_t *)(send_cond->cond));
...
static void MQTTAsync_retry(void)
{
	...
	FUNC_ENTRY;
	time(&(now));
	if (unisoc_difftime(now, last) > retryLoopInterval)
	{
		time(&(last));
