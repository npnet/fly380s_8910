...
thread_type Thread_start(thread_fn fn, void* parameter)
{
	...
	if (pthread_create(&thread, &attr, (void *)fn, parameter) != 0)
		thread = 0;
...
int Thread_stop(thread_id_type tid)
{
#if defined(WIN32) || defined(WIN64)
#else
	pthread_cancel(tid);
...
mutex_type Thread_create_mutex(void)
{
	...
	#endif
	FUNC_EXIT_RC(rc);process_compilation_warning(rc);
	return mutex;
}
 ...
 */
	...
	FUNC_ENTRY;
		...
		while (++i < count && (rc = sem_trywait((sem_t *)(*sem))) != 0)
		{
...
int Thread_signal_cond(cond_type condvar)
{
	...
	FUNC_ENTRY;
	pthread_mutex_lock((pthread_mutex_t *)condvar->mutex);
	rc = pthread_cond_signal((pthread_cond_t *)condvar->cond);
	pthread_mutex_unlock((pthread_mutex_t *)condvar->mutex);
...
int Thread_wait_cond(cond_type condvar, int timeout)
{
	...
	pthread_mutex_lock((pthread_mutex_t *)condvar->mutex);
	rc = pthread_cond_timedwait((pthread_cond_t *)condvar->cond, (pthread_mutex_t *)condvar->mutex, &cond_timeout);
	pthread_mutex_unlock((pthread_mutex_t *)condvar->mutex);
...
int Thread_destroy_cond(cond_type condvar)
{
	...
	rc = pthread_mutex_destroy((pthread_mutex_t *)condvar->mutex);
	rc = pthread_cond_destroy((pthread_cond_t *)condvar->cond);
	free(condvar);
