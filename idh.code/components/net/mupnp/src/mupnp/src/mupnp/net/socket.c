...
int getnameinfo(struct sockaddr *socketAddr, char socketLen,
                ...
                char *localPort, char localPortLen, uint16_t flag)
{
    return 0;
}
...
bool mupnp_socket_connect(mUpnpSocket *sock, const char *addr, int port)
{
    ...
    if (mupnp_socket_isssl(sock) == true)
    {
        ret = mupnp_mbedtlssocket_connect(sock, addr, port);
    }
#endif
...
* mupnp_socket_read
****************************************/
int http_socket_read(mUpnpSocket *sock, char *buffer, size_t len, int timeout_sec, int timeout_usec)
{
    mupnp_log_info("http_socket_read enter");
    int fd = sock->id;
    struct timeval tv;
    ...
    if (result < 0)
    {
        ...
        mupnp_log_info("Error in select(): %d %s\r\n");
        return result;
    }
    else if (result > 0)
    {
        int numBytes;
        mupnp_log_info("select read event happend");
        ...
        mupnp_log_info("no read event happend on this socket");
        return -1;
    }
    else
    {
        mupnp_log_info("timeout !!!");
        return -1;
    }
}
...
ssize_t mupnp_socket_read(mUpnpSocket *sock, char *buffer, size_t bufferLen)
{
...
#else
    recvLen = http_socket_read(sock, buffer, bufferLen, 20, 0);
#endif
    ...
    {
        recvLen = mbedtls_ssl_read(sock->ssl, (unsigned char *)buffer, bufferLen);
    }
#endif
...
size_t mupnp_socket_write(mUpnpSocket *sock, const char *cmd, size_t cmdLen)
{
    ...
    do
    {
        ...
        {
            nSent = mbedtls_ssl_write(sock->ssl, (const unsigned char *)(cmd + cmdPos), cmdLen);
        }
#endif
...
ssize_t mupnp_socket_readline(mUpnpSocket *sock, char *buffer, size_t bufferLen)
{
    ...
    {
        //osiThreadSleep(1);
        mupnp_log_info("mupnp_socket_readline socket read enter...\n");
        readLen = mupnp_socket_read(sock, &buffer[readCnt], sizeof(char));
        mupnp_log_info("mupnp_socket_readline socket read out...\n");
        mupnp_log_info("mupnp_socket_readline socket readLen %d...\n", readLen);
        if (readLen <= 0)
            return -1;
        readCnt++;
