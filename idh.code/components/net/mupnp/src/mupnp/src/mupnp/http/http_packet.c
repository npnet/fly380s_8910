...
//http_DL_process_handler DL_process_handler;
#if 0
void set_DL_handler(http_DL_process_handler handler){

    DL_process_handler = handler;
}
#endif
...
ssize_t mupnp_http_packet_read_chunk(mUpnpHttpPacket *httpPkt, mUpnpSocket *sock, char *lineBuf, size_t lineBufSize)
{
    ssize_t readLen = 0;
    ssize_t conLen = 0;
    ssize_t rlen = 0;
    ...
    {
        rlen = mupnp_socket_read(sock, (content + readLen), (conLen - readLen));
        if (rlen > 0)
            readLen += rlen;
    ...
    if (readLen == conLen)
    {
        ...
        /* Append content to packet */
        buf = mupnp_http_packet_appendncontent(httpPkt, content, readLen);
        if (buf == NULL)
        {
            readLen = 0;
...
bool mupnp_http_packet_read_body(mUpnpHttpPacket *httpPkt, mUpnpSocket *sock, char *lineBuf, size_t lineBufSize)
{
    ...
    if (0 < conLen)
    {
        ...
        //tempLen = 0;
        /* Read content until conLen is reached, or tired of trying */
        while (totalLen < conLen && tries < 5)
        {
            readLen = 0;
            mupnp_log_info("enter mupnp_socket_read+++ %d!\n", readLen);
            readLen = mupnp_socket_read(sock, (content + totalLen), (conLen - totalLen));
            mupnp_log_info("out mupnp_socket_read+++ %d!\n", readLen);
            ...
            totalLen += readLen;
            //if(DL_process_handler != NULL){

            //DL_process_handler(content+tempLen,conLen,totalLen,totalLen-tempLen);
            // tempLen = totalLen;

            //}
        }
        //  DL_process_handler = NULL;
        if (totalLen <= 0)
        {
            free(content);
            content = NULL;
        ...
        content[totalLen] = '\0';
        mupnp_http_packet_setcontentpointer(httpPkt, content, totalLen);
    }
