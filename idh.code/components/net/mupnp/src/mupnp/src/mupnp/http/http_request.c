...
static size_t mupnp_http_request_header_callback(void *ptr, size_t size, size_t nmemb, void *stream)
{
    ...
    /* Read header items */
    if (mupnp_strncmp(headerLine, MUPNP_HTTP_VER11, mupnp_strlen(MUPNP_HTTP_VER11)) == 0 || mupnp_strncmp(headerLine, MUPNP_HTTP_VER10, mupnp_strlen(MUPNP_HTTP_VER10)) == 0)
    {
...
{
    char lineBuf[MUPNP_HTTP_READLINE_BUFSIZE] = {0};
    ...
    if (mupnp_http_packet_hasheader((mUpnpHttpPacket *)httpReq, MUPNP_HTTP_CONTENT_LENGTH) || mupnp_http_packet_hasheader((mUpnpHttpPacket *)httpReq, MUPNP_HTTP_TRANSFER_ENCODING))
        ret = mupnp_http_packet_read_body((mUpnpHttpPacket *)httpReq, sock, lineBuf, sizeof(lineBuf));
