...
void mupnp_log_print(int severity, const char *file, int line_n, const char *function, const char *format, ...)
{
    ...
    prefix_length = snprintf(log_line, MAX_LOG_STRING, "%s%s%s%s%d%s%s%s ", map_severity(severity), separator, file, separator, line_n, separator, function, separator);
    ...
    //int nIndex = ffs(severity);
    if (severity == SEV_ERROR)
    {
        //SXS_TRACE(_AT | TNB_ARG(0) | TSTDOUT, log_line);
    }
    else
    {
        //SXS_TRACE(_AT | TNB_ARG(0) | TLEVEL(nIndex), log_line);
    }
}
#endif
