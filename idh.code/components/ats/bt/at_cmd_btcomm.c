...
#define ATC_MAX_BT_SEND_DATA_LEN 256
...
void AT_BTApp_CmdFunc_BtSpp(atCommand_t *pParam)
{
    ...
    case AT_CMD_SET:
    {
        ...
        case 0: //"SEND"
        {
            ...
            datalen = atParamUintInRange(pParam->params[2], 0, 256, &paramok);
            if (!paramok)
            {
            ...
            if (type == 0)
            {
                ...
                while (datalen - SendByte != 0)
                    SendByte += BT_SppWriteData(data + SendByte, datalen - SendByte);
            }
            else
            {
                ...
                while (datalen - SendByte != 0)
                    SendByte += BT_SppWriteData(datahex + SendByte, datalen - SendByte);
            }
