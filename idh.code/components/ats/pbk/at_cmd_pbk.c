...
void AT_PBK_ReadMEEntry(uint16_t nStartIndex, uint16_t nEndIndex, atCommand_t *cmd, uint8_t nSim)
{
    ...
    uint32_t iRet;
    NV_ME_PHB_INFOV2 sEntryForME = {0};
...

    {
        ...
        iRet = CFW_NV_READ_ME_PBK(i, pMeBuff);
        if (iRet == ERR_SUCCESS)

        {
            CFW_Buf2PBKI(pMeBuff, &sEntryForME);
                         ...
                         uint8_t *pNameBuf, uint16_t nNameLen, atCommand_t *cmd, uint8_t nSim)
{
    ...
    CFW_PBKI2Buf(&sEntryForME, pMeBuff);

    iRet = CFW_NV_WRITE_ME_PBK(nIndex, pMeBuff, NV_ME_WRITE_ADD);
    if (iRet == ERR_SUCCESS)
        RETURN_OK(cmd->engine);
    else
    {
...
void AT_PBK_DeleteMEEntry(uint16_t nIndex, atCommand_t *cmd, uint8_t nSim)
{
    ...
    iRet = CFW_NV_WRITE_ME_PBK(nIndex, pMeBuff, NV_ME_WRITE_DEL);
    if (iRet == ERR_SUCCESS)
        RETURN_OK(cmd->engine);
    else
    {
...
void atCmdHandleCPBS(atCommand_t *cmd)
{
    ...
    else if (cmd->type == AT_CMD_SET) //+CPBS=<storage>[,<password>]
    {
        ...
        {
            int32_t usenum = CFW_NV_GET_STORGE_ME_PBK(MAX_ME_PHONE_ENTRIES);
            if (usenum < 0)
            {
                RETURN_CME_ERR(cmd->engine, ERR_AT_CME_PARAM_INVALID);
            }
