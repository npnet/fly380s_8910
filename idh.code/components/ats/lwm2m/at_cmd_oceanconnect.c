...
static void cdp_server_ind_process(osiEvent_t *pEvent)
{
    ...
    OSI_LOGI(0, "cdp_server_ind_process pEvent->nEventId=%d,gOceanConnectRef=%d", pEvent->id, gOceanConnectRef);
...
void AT_CmdFunc_NCDPOPEN(atCommand_t *pParam)
{
    if (AT_CMD_SET == pParam->type)
    {
        ...
        int nImeiLen = nvmReadImei(atCmdGetSim(pParam->engine), (nvmImei_t *)imei);
        if (nImeiLen == -1)
