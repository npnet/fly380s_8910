...
#endif
    osiTimerStart(g_socketLedTimer, 1000);
}

void socketLed_startFlicker(atCmdEngine_t *engine)
{
    if (isSocketLedFlicker == 1)
    {
        OSI_LOGI(0, "socketLed_startFlicker,led already flickered");
        return;
    }
...
#ifdef CONFIG_SOC_6760
    drvTurnOnSocketLed(isSocketLedFlicker);
#endif
}
static void tcpip_rsp(void *param)
{
    ...
    case EV_CFW_TCPIP_ACCEPT_IND:
    {
        ...
        int8_t newSocket = CFW_TcpipSocketAccept(uSocket, &addr, &addr_len);
        if (newSocket <= 0)
        {
        ...
        {
            if (g_uMynetContexts.nTcpipParas[i].uState == MYNET_CLOSED && g_uMynetContexts.nTcpipParas[i].uServerState == MYNETSERVER_CLOSE)
            {
                g_uMynetContexts.nTcpipParas[i].uMode = 0;
                g_uMynetContexts.nTcpipParas[i].isText = tcpipParas->isText;
                g_uMynetContexts.nTcpipParas[i].uState = MYNET_CONNECTED;
                g_uMynetContexts.nTcpipParas[i].uSocket = newSocket;
                AT_StrNCpy(g_uMynetContexts.nTcpipParas[i].uaIPAddress, ip4addr_ntoa((ip4_addr_t *)&(addr.sin_addr)), AT_MYNET_TCPIP_HOSTLEN);
                g_uMynetContexts.nTcpipParas[i].uaIPAddress[AT_MYNET_TCPIP_HOSTLEN - 1] = '\0';
                g_uMynetContexts.nTcpipParas[i].uPort = ntohs(addr.sin_port);
                g_uMynetContexts.nTcpipParas[i].listening_socket = uSocket;
                g_uMynetContexts.nTcpipParas[i].uCid = tcpipParas->uCid;
                ...
                dnsparam->nMuxIndex = i;
                CFW_TcpipSocketSetParam(newSocket, tcpip_rsp, (uint32_t)dnsparam);
                ...
                CFW_TcpipSocketGetsockname(g_uMynetContexts.nTcpipParas[i].uSocket, &stLocalAddr, (int *)(&addrlen));
        ...
        if (g_mynetUrc == 1)
        {
            sprintf(uaRspStr, "$MYURCCLIENT:%d,\"%s\",%d", i, g_uMynetContexts.nTcpipParas[i].uaIPAddress, g_uMynetContexts.nTcpipParas[i].uPort);
            atCmdRespInfoText(engine, uaRspStr);
        }
    }
    break;
    case EV_CFW_TCPIP_SOCKET_CLOSE_RSP:
    {
        ...
        if (i >= CHANNEL_NUM_MAX) //all closed
        {
            if (g_socketLed == 1 && isSocketLedFlicker == 1)
            {
                socketLed_stopFlicker();
    ...
    case EV_CFW_TCPIP_REV_DATA_IND:
        ...
        if (tcpipParas->uSocket == uSocket)
        {
            ...
            {
                if ((tcpipParas->uMode != 1) || (tcpipParas->uState == MYNET_ACCEPT))
                {
    ...
    default:
        ...
        //char AtRet[128] = {0};
        //sprintf(AtRet, "ERROR:%d", ERR_AT_CME_OTHOR_ERROR);
        //atCmdRespErrorText(engine, AtRet);
        break;
    }
    free(pEvent);
...
void AT_GPRS_CmdFunc_MYNETACT(atCommand_t *cmd)
{
    ...
    {
        memset(AtRet, 0, sizeof(AtRet));
        sprintf(AtRet, "$MYNETACT:<status>,<Channel>");
        atCmdRespInfoText(cmd->engine, AtRet);
        RETURN_OK(cmd->engine);
    }
    else
    ...
    if (AT_CMD_SET == cmd->type)
    {
        ...
        if (refuse == 1)
        {
            ...
            if (iRetValue == ERR_SUCCESS)
            {
                ...
                //start_tcpip_close_timer(cmd->engine, uMuxIndex);
                return;
            }
            else
            {
...
void AT_TCPIP_CmdFunc_MYNETCON(atCommand_t *cmd)
{
    ...
    if (AT_CMD_SET == cmd->type)
    {
        ...
        else if (stricmp(type, "CFGT") == 0) //transparent mode time 1-65535ms/default 100ms
        {
            ...
            gChannelParam[uCid].trans_waittm = WaitTm;
            RETURN_OK(cmd->engine);
        }
        else if (stricmp(type, "CFGP") == 0) //transparent mode 1-1460/default 1024
        {
            ...
            gChannelParam[uCid].trans_sendSz = SendSz;
            RETURN_OK(cmd->engine);
        }
    ...
    else if (AT_CMD_READ == cmd->type)
    {
        for (uCid = AT_PDPCID_MIN; uCid <= 6; uCid++)
        {
            ...
            tmpStr += sprintf(tmpStr, "CFGT,%ld,", gChannelParam[uCid].trans_waittm);

            tmpStr += sprintf(tmpStr, "CFGP,%ld,", gChannelParam[uCid].trans_sendSz);
    ...
    if (AT_CMD_SET == cmd->type)
    {
        ...
        if (paramok)
        {
            ...
            if (mode == 1)
            {
                ...
                memset(tcpipParas, 0, sizeof(stAT_Tcpip_Paras));
                gMynetlocalport[uMuxIndex].tcp_port = atoi(port);
...
void AT_TCPIP_CmdFunc_MYNETOPEN(atCommand_t *cmd)
{
    ...
    if (AT_CMD_SET == cmd->type)
    {
        ...
        if (tcpipParas->uMode == 1)
        {
            ...
            {
                AT_CMD_RETURN(atCmdRespErrorCode(cmd->engine, ERR_AT_CME_TCPIP_ALREADY_CONNECTED));
            }
            tcpipParas->isTrans = 0;
        ...
        for (int i = 0; i < MYNET_NUM_NETCONN; i++)
        {
            ...
            if (tcpipParas->uState != MYNET_CLOSED || tcpipParas->uServerState != MYNETSERVER_CLOSE)
            {
                ...
                if (tcpipParas->uProtocol == CFW_TCPIP_IPPROTO_UDP)
                    ...
                    gMynetlocalport[i].tcp_port = ntohs(stLocalAddr.sin_port);
                sprintf(aucBuffer, "$MYNETOPEN: %d,%s,%d,%s,%s,%s,%d,%s,%d", i, tcpipParas->uLocalip, (tcpipParas->uProtocol == CFW_TCPIP_IPPROTO_UDP) ? gMynetlocalport[i].udp_port : gMynetlocalport[i].tcp_port, ipaddr_ntoa(&(netif->gw)), sPdpCont.nPdpDnsSize == 0 ? "0.0.0.0" : ip4addr_ntoa(&ip4_dns1), sPdpCont.nPdpDnsSize == 0 ? "0.0.0.0" : ip4addr_ntoa(&ip4_dns2), tcpipParas->uMode, tcpipParas->uaIPAddress, tcpipParas->uPort);
...
static void _sendTimeout(atCommand_t *cmd)
{
...
static void _tcpipSend(atCommand_t *cmd, uint32_t uMuxIndex, uint8_t *send_data, uint32_t uLength)
{
    ...
    else
    {
        if (!mynetMux_multiIp)
            ...
            sprintf(aucBuffer, "ERROR:%d", ERR_AT_CME_OTHOR_ERROR);
...
void AT_TCPIP_CmdFunc_MYNETURC(atCommand_t *cmd)
{
    ...
    if (cmd->type == AT_CMD_SET)
    {
        ...
        uint32_t enable = atParamDefUintInRange(cmd->params[0], 1, 0, 1, &paramok);
        if (!paramok)
        {
            sprintf(aucBuffer, "ERROR:%d", ERR_AT_CME_PARAM_ERROR);
            AT_CMD_RETURN(atCmdRespErrorText(cmd->engine, aucBuffer));
...
void AT_TCPIP_CmdFunc_MYSOCKETLED(atCommand_t *cmd)
{
    ...
    if (cmd->type == AT_CMD_SET)
    {
        ...
        uint8_t socketLed = atParamDefUintInRange(cmd->params[0], 1, 0, 1, &paramok);
        if (!paramok)
        {
            sprintf(aucBuffer, "ERROR:%d", ERR_AT_CME_PARAM_ERROR);
            AT_CMD_RETURN(atCmdRespErrorText(cmd->engine, aucBuffer));
