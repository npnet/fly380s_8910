...
static void tcpip_connect_done(atCmdEngine_t *engine, uint8_t nMuxIndex)
{
    ...
    else
    {
        ...
        if (!cipMux_multiIp)
            ...
            sprintf(uaRspStr, "%d,CONNECT OK", nMuxIndex);
...
static void tcpip_rsp(void *param)
{
    ...
    case EV_CFW_TCPIP_ACCEPT_IND:
    {
        ...
        SOCKET newSocket = CFW_TcpipSocketAccept(uSocket, (CFW_TCPIP_SOCKET_ADDR *)&addr, &addr_len);
        if (newSocket < 0)
        {
        ...
        for (i = 0; i < MEMP_NUM_NETCONN; i++)
        {
            if (g_uCipContexts.nTcpipParas[i].uState == CIP_INITIAL)
            {
                ...
                g_uCipContexts.nTcpipParas[i].engine = engine;
                g_uCipContexts.nTcpipParas[i].listening_socket = uSocket;
                gCipserver.channelid = i;
                ip_addr_t from_addr_t;
                u16_t local_port;
                SOCKADDR_TO_IPADDR_PORT(((const struct sockaddr *)&addr), &from_addr_t, local_port);
                strncpy(g_uCipContexts.nTcpipParas[i].uaIPAddress, ipaddr_ntoa(&from_addr_t), AT_TCPIP_HOSTLEN);
                g_uCipContexts.nTcpipParas[i].uPort = local_port;
                ...
                tcpparam->nMuxIndex = i;
                CFW_TcpipSocketSetParam(newSocket, tcpip_rsp, (uint32_t)tcpparam);
        ...
        if (i >= MEMP_NUM_NETCONN)
            OSI_LOGI(0, "no socket index to use!");
        else
        {

            sprintf(uaRspStr, "%d, REMOTE IP: %s", gCipserver.channelid, g_uCipContexts.nTcpipParas[i].uaIPAddress);
            atCmdRespUrcText(engine, uaRspStr);
        }
    }
    break;
...
void AT_TCPIP_CmdFunc_CIICR(atCommand_t *cmd)
{
    ...
    if (cmd->type == AT_CMD_EXE)
    {
        ...
        if (gOutsidePdpCid == 0 && useOutsidePdp(cmd))
        {
            bearerParas->nState = CIP_GPRSACT;
            RETURN_OK(cmd->engine);
        }
...
void AT_TCPIP_CmdFunc_CIPSTATUS(atCommand_t *cmd)
{
    ...
    else if (cmd->type == AT_CMD_EXE)
    {
        ...
        {
            atCmdRespUrcText(cmd->engine, "OK");
            sprintf(aucBuffer, "STATE:%s", g_strATTcpipStatus[bearer_state]);
            atCmdRespUrcText(cmdEngine, aucBuffer);

            if (gCipserver.mode == CIPSERVER_OPEN)
            {
                sprintf(aucBuffer, "S:0,0,\"%ld\",\"%s\"", gCipserver.port, "LISTENING");
                atCmdRespUrcText(cmdEngine, aucBuffer);
            }

            for (int i = 0; i < MEMP_NUM_NETCONN - (CIPSERVER_OPEN == gCipserver.mode); i++)
            {
                sprintf(aucBuffer, "C:%d,%d,%s,%s,%d,%s", i,
