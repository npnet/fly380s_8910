...
static void prvHButtonDetectWork(void *ctx)
{
    ...
    {
        hmic_channel_adc_value = drvPmicAudGetHmicAdcValue();
        if (d->notify_cb != NULL)
    ...
    {
        if (d->notify_cb != NULL)
            d->notify_cb(d->notify_cb_ctx, MSG_HEADSET_BTN_UP, 0);
    }
}
...
static void prvHeadsetDetectWork(void *ctx)
{
    ...
    if (d->headset_flag)
    {
        if (d->s_headset_ctl.status == HEADSET_DISCONNECT)
        {
            ...
            if ((d->type == DRV_HEADSET_TYPE_4POLE_NORMAL) || (d->type == DRV_HEADSET_TYPE_4POLE_NOT_NORMAL))
            {
                drvPmicEicTrigger(DRV_PMIC_EIC_AUDIO_HEAD_BUTTON, HEADSET_BUTTON_DEBOUNCE_TIME, true);
            }
            else
...
void drvDummyHeadsetCustCB(void *ctx, drvHeadsetNotifyMsg_t id, uint32_t param)
{
    char rsp[64];
    ...
    case MSG_HEADSET_BTN_DOWN:
        sprintf(rsp, "+HEADBTN DOWN: %d", (int)param);
        //atCmdRespDefUrcText(rsp);
        OSI_LOGXI(OSI_LOGPAR_S, 0, "%s\n", rsp);
        break;

    case MSG_HEADSET_BTN_UP:
        sprintf(rsp, "+HEADBTN UP");
        //atCmdRespDefUrcText(rsp);
        OSI_LOGXI(OSI_LOGPAR_S, 0, "%s\n", rsp);
        break;
    }
...
void drvHandsetInit(void)
{
    ...
    //dummyCustCb
    drvHandsetSetNotifyCustCB(drvDummyHeadsetCustCB, d);
