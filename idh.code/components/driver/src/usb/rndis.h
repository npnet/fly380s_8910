...
struct rndis_resp;
typedef TAILQ_HEAD(rndis_resp_head, rndis_resp) rndisRespHead_t;
typedef TAILQ_ENTRY(rndis_resp) rndisRespEntry_t;

typedef struct rndis_resp
{
    rndisRespEntry_t anchor;
    uint32_t length;
    uint8_t *buf;
} rndisResp_t;

typedef struct rndis_params rndisParams_t;
typedef void (*responseAvail_t)(void *param);

/* RNDIS Message parser and other useless functions */
int rndis_msg_parser(struct rndis_params *params, uint8_t *buf);
rndisParams_t *rndis_register(responseAvail_t resp_cb, void *param);
void rndis_deregister(struct rndis_params *params);
int rndis_set_param_dev(struct rndis_params *params, rndisData_t *ether,
                        uint16_t *cdc_filter);
int rndis_set_param_vendor(struct rndis_params *params, uint32_t vendorID,
