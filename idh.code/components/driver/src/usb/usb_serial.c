...
{
    osiSemaphoreTryAcquire(priv->tx_finish_sem, 0);
    if (prvIsTxFinishedLocked(priv))
        return true;
    ...
    osiSemaphoreTryAcquire(priv->tx_finish_sem, timeout);
...
static bool prvWaitWriteAvailLocked(usbSerialPriv_t *priv, unsigned timeout)
{
    ...
    osiSemaphoreTryAcquire(priv->tx_avail_sem, 0);
    ...
    osiSemaphoreTryAcquire(priv->tx_avail_sem, timeout);
...
static bool prvWaitReadAvailLocked(usbSerialPriv_t *priv, unsigned timeout)
{
    ...
    osiSemaphoreTryAcquire(priv->rx_avail_sem, 0);
    if (!osiBlockedFifoIsEmpty(priv->rx_fifo))
        return true;

    osiSemaphoreTryAcquire(priv->rx_avail_sem, timeout);
...
static bool prvWaitWriteAvail(drvSerialImpl_t *port, unsigned timeout)
{
    ...
    if (!SERIAL_RUNNING(priv) || !priv->open)
        return false;
    return prvWaitWriteAvailLocked(priv, timeout);
}
...
static bool prvWaitReadAvail(drvSerialImpl_t *port, unsigned timeout)
{
    ...
    if (!SERIAL_RUNNING(priv))
        return false;
    return prvWaitReadAvailLocked(priv, timeout);
}
...
static bool prvWaitWriteFinish(drvSerialImpl_t *port, unsigned timeout)
{
    ...
    if (!SERIAL_RUNNING(priv) || !priv->open)
        return false;
    return prvWaitWriteFinishLocked(priv, timeout);
}

static void prvFreePriv(usbSerialPriv_t *priv, udc_t *controller)
{
    priv->inited = false;
