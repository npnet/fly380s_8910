...
uint32_t _sprd_codec_ap_headmic_adcget(uint32_t is_big_scale)
{
    ...
    adc_Value = drvAdcGetRawValue(ADC_CHANNEL_HEADMIC, (is_big_scale) ? ADC_SCALE_3V233 : ADC_SCALE_1V250);
    OSI_LOGI(0, "adc_value is %d\n", adc_Value);
    sc2720m_ana_hdt0.b.rg_aud_hedet_buf_chop = 1;
    halAdiBusWrite(&hwp_rda2720mAudCodec->ana_hdt0, sc2720m_ana_hdt0.v);

    _sprd_codec_set_auxadc_scale(is_big_scale);
    adc_Value = drvAdcGetRawValue(ADC_CHANNEL_HEADMIC, (is_big_scale) ? ADC_SCALE_3V233 : ADC_SCALE_1V250) + adc_Value;
    OSI_LOGI(0, "adc_value is %d\n", adc_Value / 2);
    return adc_Value / 2;
}
...
static int _sprd_codec_ap_headmic_get_adcideal(uint32_t adc_mic, uint32_t coefficient)
{
    int64_t numerator = 0;
    int64_t denominator = 0;
    uint32_t adc_ideal = 0;
    uint32_t a, b, e1, e2;
    ...
    {
        OSI_LOGI(0, "headset: efuse A,B,E hasn't been calculated!\n");
        return adc_mic;
    }
    ...
    OSI_LOGI(0, "exp1=%d, exp2=%d, exp3=%d, exp4=%d\n", exp1, exp2, exp3, exp4);
    denominator = exp3 + exp4;
    numerator = coefficient * (exp1 + 1200) * exp2;

    OSI_LOGI(0, "denominator=%d, numerator=%d\n", denominator, numerator);
    ...
    */
    ...
    numerator = numerator / 100;
    OSI_LOGXI(OSI_LOGPAR_DD, 0, "denominator=%lld, numerator=%lld\n", denominator, numerator);

    divisor = (uint32_t)(denominator);
    dividend = (uint64_t)(numerator);
    OSI_LOGXI(OSI_LOGPAR_ID, 0, "divisor=%u, dividend=%llu\n", divisor, dividend);

    //do_div(dividend, divisor);
    dividend = dividend / divisor;
    adc_ideal = (uint64_t)dividend;
    OSI_LOGI(0, "adc_mic=%d, adc_ideal=%d\n", adc_mic, adc_ideal);
...
void _sprd_codec_headset_detect_reg_init(void)
{
    ...
    /* headset_reg_set_bits(ANA_HDT0, BIT(HEDET_BUF_CHOP)); */
    {
        ...
        halAdiBusWrite(&hwp_rda2720mAudCodec->ana_hdt0, sc2720m_ana_hdt0.v);
        OSI_LOGXI(OSI_LOGPAR_SII, 0, "[%s][%d]ana_hdt0: 0x%08x\n", __FUNCTION__, __LINE__, sc2720m_ana_hdt0.v);
    }
    ...
    sc2720m_ana_hdt1.v = halAdiBusRead(&hwp_rda2720mAudCodec->ana_hdt1);
    OSI_LOGXI(OSI_LOGPAR_SII, 0, "[%s][%d]ana_hdt1: 0x%08x\n", __FUNCTION__, __LINE__, sc2720m_ana_hdt1.v);
    ...
    sc2720m_ana_hdt2.v = halAdiBusRead(&hwp_rda2720mAudCodec->ana_hdt2);
    OSI_LOGXI(OSI_LOGPAR_SII, 0, "[%s][%d]ana_hdt2: 0x%08x\n", __FUNCTION__, __LINE__, sc2720m_ana_hdt2.v);
    ...
    REG_RDA2720M_AUD_CODEC_ANA_DCL0_T sc2720m_ana_dcl0;
    sc2720m_ana_dcl0.v = halAdiBusRead(&hwp_rda2720mAudCodec->ana_dcl0);
    OSI_LOGXI(OSI_LOGPAR_SII, 0, "[%s][%d]ana_dcl0: 0x%08x\n", __FUNCTION__, __LINE__, sc2720m_ana_dcl0.v);
    ...
    halAdiBusWrite(&hwp_rda2720mAudCodec->ana_pmu0, sc2720m_ana_pmu0.v);
    sc2720m_ana_pmu0.v = halAdiBusRead(&hwp_rda2720mAudCodec->ana_pmu0);
    OSI_LOGXI(OSI_LOGPAR_SII, 0, "[%s][%d]ana_pmu0: 0x%08x\n", __FUNCTION__, __LINE__, sc2720m_ana_pmu0.v);
}
...
SC2720_HEADSET_TYPE_T __sprd_codec_ap_headmic_type_detect(void)
{
    ...
    OSI_LOGXI(OSI_LOGPAR_SI, 0, "[%s][%d]\n", __FUNCTION__, __LINE__);
    sc2720m_ana_hdt0.v = halAdiBusRead(&hwp_rda2720mAudCodec->ana_hdt0);
    OSI_LOGXI(OSI_LOGPAR_SII, 0, "[%s][%d]ana_hdt0: %x\n", __FUNCTION__, __LINE__, sc2720m_ana_hdt0.v);
    ...
    while (retry_times > 0)
    {
        ...
        {
            OSI_LOGI(0, "headset: retry time is %d adc_mic_average=%d<headset_stable_max_value ",
                     retry_times, adc_mic_average);
            break;
        }
    }
    ...
    OSI_LOGI(0, "headset: now get adc value  in little scale\n");
    ...
    OSI_LOGI(0, "headset: adc_mic_average2 = %d\n", adc_mic_average);
#if 0
    ...
    else if (((adc_mic_average > adc_left_average) ? (adc_mic_average - adc_left_average) : (adc_left_average - adc_mic_average)) < cfg_ptr->headset_adc_gnd) // 0.1v
    {
...
uint32_t __sprd_codec_headset_type_detect(void)
{
    ...
    OSI_LOGXI(OSI_LOGPAR_SII, 0, "[%s][%d]heaset_type: %x\n", __FUNCTION__, __LINE__, heaset_type);
    return heaset_type;
}
...
uint32_t __sprd_codec_ap_get_headbutton_value(void)
{
    uint32_t adc_mic_average = 0;
    uint32_t adc_mic_ideal = 0;
    //int val;
    ...
    {
        OSI_LOGXI(OSI_LOGPAR_SI, 0, "[%s][%d]headphone not connect\n", __FUNCTION__, __LINE__);
        return 0;
    }
    ...
    adc_mic_ideal = _sprd_codec_ap_headmic_get_adcideal(adc_mic_average,
                                                        cfg_ptr->coefficient);
    OSI_LOGXI(OSI_LOGPAR_SIII, 0, "[%s][%d]adc_mic_average= %d adc_mic_ideal = %d\n", __FUNCTION__, __LINE__, adc_mic_average, adc_mic_ideal);
