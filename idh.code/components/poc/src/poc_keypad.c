...
static void prvPowerKeyCb(void *ctx)
{
	...
	if(!lv_poc_charge_poweron_status())//正常开机
	{
		...
		{
			OSI_PRINTFI("[poc][loginout](%s)(%d):send success", __func__, __LINE__);
		}
	...
	if(id == LV_GROUP_KEY_POC) //poc
	{
		if(prvPttKeyState != state
			&&(lv_poc_get_current_activity() == activity_idle
				...
				|| lv_poc_build_tempgrp_progress(POC_TMPGRP_READ) == POC_TMPGRP_FINISH))
		{
			if(state == LV_INDEV_STATE_PR)
			{
				switch(lv_poc_get_apply_note())
				{
					...
					{
						lv_poc_cit_get_run_status() == LV_POC_CIT_OPRATOR_TYPE_KEY ? lv_poc_type_key_poc_cb(true) : (lv_poc_get_loopback_recordplay_status() ? lvPocLedCom_Msg(LVPOCGUICOM_SIGNAL_LOOPBACK_RECORDER_IND, false) : lvPocGuiBndCom_Msg(LVPOCGUIBNDCOM_SIGNAL_SPEAK_START_IND, NULL));
						break;
					}
				}
            }
