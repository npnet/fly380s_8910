			...
			switch(pocOemAttr.loginstatus_t)
			{
				case LVPOCOEMCOM_SIGNAL_LOGIN_SUCCESS:
				{
					...
					else if(NULL != strstr(apopt->oembuf,LVPOCPOCOEMCOM_SIGNAL_OPTCODE_STARTLISTEN_ACK))
					{
						...
						lvPocGuiOemCom_Msg_Listen_Speak(LVPOCGUIOEMCOM_SIGNAL_LISTEN_SPEAKER_REP, NULL);
						break;
					}
					//listen stop--8B0000应答
					else if(NULL != strstr(apopt->oembuf,LVPOCPOCOEMCOM_SIGNAL_OPTCODE_STOPLISTEN_ACK))
					{
						lvPocGuiOemCom_Msg_Listen_Speak(LVPOCGUIOEMCOM_SIGNAL_LISTEN_STOP_REP, NULL);
						break;
					}
					//speak start--0B应答
					else if(NULL != strstr(apopt->oembuf, (char *)LVPOCPOCOEMCOM_SIGNAL_OPTCODE_STARTSPEAK_ACK))
					{
						...
						lvPocGuiOemCom_Msg_Listen_Speak(LVPOCGUIOEMCOM_SIGNAL_SPEAK_START_REP, NULL);
						break;
					}
					//speak stop--0C应答
					else if(NULL != strstr(apopt->oembuf,(const char *)LVPOCPOCOEMCOM_SIGNAL_OPTCODE_STOPSPEAK_ACK))
					{
						lvPocGuiOemCom_Msg_Listen_Speak(LVPOCGUIOEMCOM_SIGNAL_SPEAK_STOP_REP, NULL);
						break;
					}
				...
				case LVPOCOEMCOM_SIGNAL_LOGIN_FAILED:
				{
					...
					//network connect
					else if(NULL != strstr(apopt->oembuf,LVPOCPOCOEMCOM_SIGNAL_OPTCODE_NETWORK_CONNECTED_ACK))
					{
						lvPocGuiOemCom_Request_PocParam();
						break;
					}
					...
					else if(NULL != strstr(apopt->oembuf,LVPOCPOCOEMCOM_SIGNAL_OPTCODE_GETPARAM_ACK))
					{
						...
						else//需重新设置账号
						{
							...
							osiTimerStartRelaxed(pocOemAttr.Oem_AutoLogin_timer, 3000, OSI_WAIT_FOREVER);
							OSI_LOGI(0, "[oemack][login]accout error, please to set");
						}
						break;
					}
					//login success
	...
	switch(id)
	{
		case LVPOCGUIOEMCOM_SIGNAL_JOIN_GROUP_REP://86
		{
			...
			if(0 == strcmp((char *)pocOemAttr.OemCurrentGroupInfo.m_ucGID,LVPOCPOCOEMCOM_SIGNAL_OPTCODE_EXIT_GROUP))//exit group
			{
				...
				char cOutstr[256] = {0};
				cOutstr[0] = '\0';
				sprintf(cOutstr, "[grouprefr][oemsinglecall]%s(%d):exit group", __func__, __LINE__);
				OSI_LOGI(0, cOutstr);
				#endif
			}
			else
...
#if GUIIDTCOM_OEM_SELF_MSG
			lvPocGuiOemCom_set_response_speed(LVPOCOEMCOM_TYPE_REPPONSE_SPEED_GROUPLIST);
#endif
			break;
		}
...
void prvPocGuiOemTaskHandleMemberList(uint32_t id, uint32_t ctx)
{
	switch(id)
	{
...
#if GUIIDTCOM_OEM_SELF_MSG
					lvPocGuiOemCom_set_response_speed(LVPOCOEMCOM_TYPE_REPPONSE_SPEED_MEMBERLIST);
#endif
				}
				break;
			}
		...
		case LVPOCGUIOEMCOM_SIGNAL_MEMBERLIST_DATA_REP:
		{
			...
			{
				//Nodelink
				OSI_LOGI(0,"[tian]prv_lv_poc_oem_get_group_member_info(apopt->oembuf)\n");
...
#if GUIIDTCOM_OEM_SELF_MSG
	lvPocGuiOemCom_set_response_speed(LVPOCOEMCOM_TYPE_REPPONSE_SPEED_MONITORGROUP);
#endif
	lvPocGuiOemCom_Msg(LVPOCGUIOEMCOM_SIGNAL_POWERON_CHECK_MONITOR_GROUP_REP, NULL);
}
...
void pocGuiOemComTaskEntry(void *argument)
{
	...
	lvPocGuiOemCom_Request_Set_Volum("64");//set volum:0-FFFF
	lvPocGuiOemCom_Request_Set_Tone("01");//open tone
    while(1)
    {
...
void prv_lv_poc_oem_get_group_member_info(void *information)
{
	...
	char cOutstr[256] = {0};
	cOutstr[0] = '\0';
	sprintf(cOutstr, "[tian][3]%s(%d):groupxmemberlistbuf.buf[5].oembuf[%s]", __func__, __LINE__, groupxmemberlistbuf.buf[5].oembuf);
	OSI_LOGI(0, cOutstr);
...
PocGuiOemAtBufferAttr_t prv_lv_poc_oem_get_global_atcmd_info(void *info, int index, void *table)
{
	...
	for(int i = 0; i < index; i++)
	{
		if(i == (index - 1))
		{
			...
			#if 1
			OSI_LOGXI(OSI_LOGPAR_SI, 0, "[tian]grouplistinfo %s", oematbuf.buf[i].oembuf);
			#endif
			break;
		}
