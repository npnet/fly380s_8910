...
extern void pub_lv_poc_set_watchdog_status(int status);
		...
		case LVPOCGUIOEMCOM_SIGNAL_AP_POC_REP:
		{
			...
			switch(pocOemAttr.loginstatus_t)
			{
				case LVPOCOEMCOM_SIGNAL_LOGIN_SUCCESS:
				{
					...
					//call note info--notify ack:84
					else if(NULL != strstr(apopt->oembuf,(const char *)LVPOCPOCOEMCOM_SIGNAL_OPTCODE_NOTE_INFO_ACK))
					{
						lvPocGuiOemCom_Msg(LVPOCGUIOEMCOM_SIGNAL_SINGLE_CALL_STATUS_EXIT_REP, (void *)1);
						break;
					}
...
void prvPocGuiOemTaskHandleGroupUpdateOpt(uint32_t id, uint32_t ctx)
{
	switch(id)
	{
		case LVPOCGUIOEMCOM_SIGNAL_JOIN_GROUP_REP://86
		{
				...
				switch(pocOemAttr.call_type)
				{
					case LVPOCOEMCOM_CALL_TYPE_SINGLE:
					{
						if(pocOemAttr.signal_multi_call_type == USER_OPRATOR_SIGNAL_CALL)
						{
							...
							lvPocGuiOemCom_Msg(LVPOCGUIOEMCOM_SIGNAL_SINGLE_CALL_STATUS_EXIT_REP, (void *)2);
						}
						else
						{
			...
			{
				pocOemAttr.is_group_in = true;
				if(0 == strcmp((char *)pocOemAttr.OemCurrentGroupInfo.m_ucGTemporary, LVPOCPOCOEMCOM_SIGNAL_OPTCODE_TEMPORARYGROUP))
				{
					//Get robbed of a speech
					if(lvPocGuiOemCom_get_speak_status())
					{
						OSI_PRINTFI("[group](%s)(%d)steal's speak", __func__, __LINE__);
						pocOemAttr.is_speak_status = false;
				...
				else if(0 == strcmp((char *)pocOemAttr.OemCurrentGroupInfo.m_ucGTemporary, LVPOCPOCOEMCOM_SIGNAL_OPTCODE_FIXEDGROUP))
				{
					...
					if((pub_lv_poc_get_watchdog_status()&0x2) == 0x2)
					{
						int status = pub_lv_poc_get_watchdog_status()&0x5;
						pub_lv_poc_set_watchdog_status(status);
					}
					else
					{
...
void prvPocGuiOemTaskHandleGroupList(uint32_t id, uint32_t ctx)
{
	switch(id)
	{
		case LVPOCGUIOEMCOM_SIGNAL_GROUP_LIST_QUERY_IND:
		{
			...
			OSI_PRINTFI("[grouplist](%s)(%d):goto query list", __func__, __LINE__);
			break;
		}
	    case LVPOCGUIOEMCOM_SIGNAL_GROUP_LIST_QUERY_REP:
	    {
			...
			if(0 == strcmp(Grouplist.buf[1].oembuf, LVPOCPOCOEMCOM_SIGNAL_OPTCODE_FAILED))
			{
				...
				{
					OSI_PRINTFI("[grouplist](%s)(%d):NULL", __func__, __LINE__);
				}
			...
			else
			{
				...
				if(pocOemAttr.pocGetGroupListCb != NULL)
				{
					...
					OSI_PRINTFI("[grouplist](%s)(%d):goto build a nodelist", __func__, __LINE__);
				}
				else
				{
					OSI_PRINTFI("[grouplist](%s)(%d):NULL", __func__, __LINE__);
		...
		case LVPOCGUIOEMCOM_SIGNAL_STOP_GROUP_REFR_REP:
		{
	...
	switch(id)
	{
		...
		case LVPOCGUIOEMCOM_SIGNAL_BIUILD_TEMPGRP_GET_MEMBER_REP:
		{
			if(pocOemAttr.pocBuildTempGrpCb == NULL)
			{
				...
				pocOemAttr.is_enter_signal_multi_call ? lv_poc_activity_func_cb_set.build_tmp_get_info() : 0;
			}
			else
			{
...
void prvPocGuiOemTaskHandleSetCurrentGroup(uint32_t id, uint32_t ctx)
{
	switch(id)
	{
		case LVPOCGUIOEMCOM_SIGNAL_SET_CURRENT_GROUP_IND:
		{
			...
			lv_task_t *oncetask = lv_task_create(lv_poc_activity_func_cb_set.member_list.group_member_act, 5, LV_TASK_PRIO_HIGHEST, (void *)SetCGroup->m_ucGName);
            lv_task_once(oncetask);
			break;
		}
		...
		case LVPOCGUIOEMCOM_SIGNAL_MEMBER_LIST_QUERY_REP:
		{
			...
			if(pocOemAttr.is_enter_signal_multi_call)//signal call or multi call
			{
				...
				if(pocOemAttr.Msg_GroupMemberData_s.dwNum > 1)//multi call(no self)
				{
					...
					if(pocOemAttr.Msg_GroupMemberData_s.dwNum > 5
						|| pocOemAttr.pocGetMemberListCb == NULL)//max 5
					{
						pocOemAttr.pocGetMemberListCb = NULL;
						lv_poc_activity_func_cb_set.window_note(LV_POC_NOTATION_NORMAL_MSG, (const uint8_t *)"错误参数", NULL);
						lvPocGuiOemCom_set_obtainning_state(false);
						OSI_PRINTFI("[multi-call](%s)(%d):error", __func__, __LINE__);
						break;
					}
					lv_task_t *oncetask = lv_task_create(lv_poc_activity_func_cb_set.build_tmp_open, 5, LV_TASK_PRIO_HIGHEST, (void *)pocOemAttr.OemCurrentGroupInfo.m_ucGName);
					lv_task_once(oncetask);
				...
				else//signal call
				{
					...
					if(pocOemAttr.is_member_call)
					{
						...
						lvPocGuiOemCom_Msg(LVPOCGUIOEMCOM_SIGNAL_SINGLE_CALL_STATUS_OK_REP, NULL);
						break;
					}
...
void prvPocGuiOemTaskHandleMemberStatus(uint32_t id, uint32_t ctx)
{
	switch(id)
	{
		...
		{
			if(ctx == 0
				|| pocOemAttr.pocGetMemberStatusCb == NULL)
			{
				break;
			}
...
void prvPocGuiOemTaskHandleMemberSignalCall(uint32_t id, uint32_t ctx)
{
	switch(id)
	{
		case LVPOCGUIOEMCOM_SIGNAL_SINGLE_CALL_STATUS_IND:
		{
			...
			if(member_call_config->enable == true)//request single call
			{
				if(pocOemAttr.member_call_dir == 0)
				{
					...
					OSI_PRINTFI("[oemsinglecall](%s)(%d):start request single call", __func__, __LINE__);
				}else
				{
					member_call_config->func(0, 0);
					pocOemAttr.is_member_call = true;

					OSI_PRINTFI("[oemsinglecall](%s)(%d):recive single call", __func__, __LINE__);
				}
			}
			else//exit single
			{
				...
				OSI_PRINTFI("[oemsinglecall](%s)(%d):local exit single call", __func__, __LINE__);
			}
		...
		case LVPOCGUIOEMCOM_SIGNAL_SINGLE_CALL_STATUS_OK_REP:
		{
			...
			OSI_PRINTFI("[oemsinglecall](%s)(%d):receive single call ack", __func__, __LINE__);
		...
		case LVPOCGUIOEMCOM_SIGNAL_SINGLE_CALL_STATUS_EXIT_REP:
		{
			...
			{
				pocOemAttr.is_group_in = false;
				poc_play_voice_one_time(LVPOCAUDIO_Type_Exit_Member_Call, 30, false);
	...
	switch(id)
	{
		case LVPOCGUIOEMCOM_SIGNAL_MEMBER_GROUP_INFO_UPDATE://85
		{
			...
			if(NULL != strstr(GUInfo.buf[1].oembuf, LVPOCPOCOEMCOM_SIGNAL_OPTCODE_GROUPINFOUPDATE))//组需更新
			{
				...
				osiTimerStart(pocOemAttr.Oem_GroupUpdate_timer,3000);

				OSI_PRINTFI("[grouprefr](%s)(%d):receive grouprefr request", __func__, __LINE__);
			}
			else if(NULL != strstr(GUInfo.buf[1].oembuf, LVPOCPOCOEMCOM_SIGNAL_OPTCODE_MEMEBERINFOUPDATE))//成员需更新
			{
				...
				osiTimerStart(pocOemAttr.Oem_MemberUpdate_timer,3000);

				OSI_PRINTFI("[group][refresh](%s)(%d):receive memberrefr request", __func__, __LINE__);
			}
...
static void prvPocGuiOemTaskHandleOther(uint32_t id, uint32_t ctx)
{
    switch(id)
    {
        case LVPOCGUIOEMCOM_SIGNAL_STOP_PLAYER_VOICE:
        {
			...
			if(type)
			{
				switch(type)
				{
					case 1:
					{
						lvPocGuiOemCom_tts_state(false);
						break;
					}

					case 2:
					{
						if(lvPocGuiOemCom_tts_state(2)
		...
		case LVPOCGUIOEMCOM_SIGNAL_SET_STOP_PLAYER_TTS_VOICE:
		{
			...
			{
				OSI_PRINTFI("[tts][status](%s)(%d):false", __func__, __LINE__);
				lvPocGuiOemCom_tts_state(false);
			}
			break;
		}
		...
		case LVPOCGUIOEMCOM_SIGNAL_SET_START_PLAYER_TTS_VOICE:
		{
			if(lvPocGuiOemCom_get_login_status() == LVPOCOEMCOM_SIGNAL_LOGIN_SUCCESS)
			{
				OSI_PRINTFI("[tts][status](%s)(%d):true", __func__, __LINE__);
				lvPocGuiOemCom_tts_state(true);
			}
			break;
		}
		...
		case LVPOCGUIOEMCOM_SIGNAL_CHECK_QUILITY_MODE_IND:
		{
			...
			if(NULL == strstr(QuilityMode.buf[3].oembuf, "01"))
			{
				OSI_PRINTFI("[quility][mode](%s)(%d):mode error(%s), reconfig", __func__, __LINE__, QuilityMode.buf[3].oembuf);
				nv_poc_setting_msg_t *poc_config = lv_poc_setting_conf_read();
				if(poc_config->current_sound_quality == 0)
				{
					lvPocGuiOemCom_Msg(LVPOCGUIOEMCOM_SIGNAL_SOUND_QUALITY_4K_IND, NULL);
			...
			OSI_PRINTFI("[quility][mode](%s)(%d):quilty(%s), mode(%s)", __func__, __LINE__, QuilityMode.buf[1].oembuf, QuilityMode.buf[3].oembuf);
			break;
		}

		case LVPOCGUIOEMCOM_SIGNAL_VOICE_PLAY_START_IND:
		{
		...
		case LVPOCGUIOEMCOM_SIGNAL_VOICE_PLAY_STOP_IND:
		{
...
void pocGuiOemComTaskEntry(void *argument)
{
	...
	lvPocGuiOemCom_Request_Set_Log(OEM_FUNC_OPEN);
...
int lvPocGuiOemCom_tts_state(int status)
{
	if(status == 0)
	{
		...
		pocOemAttr.ttsstatus = false;
	}
	else if(status == 1)
	{
		OEM_TTS_Status_CB(true);
		pocOemAttr.ttsstatus = true;
	}
	else
	{
		return pocOemAttr.ttsstatus;
	}
	return 2;
}
