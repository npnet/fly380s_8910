...
void lv_poc_shutdown_charge_power_on_logo(void)
{
	...
	//魔方图片
	lv_poc_refr_task_once(lv_poc_show_sprd_image, LVPOCLISTIDTCOM_LIST_PERIOD_10, LV_TASK_PRIO_MID);
	osiThreadSleep(2000);
	...
	//电池图片
	lv_poc_refr_task_once(lv_poc_show_first_battery_image, LVPOCLISTIDTCOM_LIST_PERIOD_10, LV_TASK_PRIO_MID);
...
lv_img_dsc_t *lv_poc_power_on_charge_get_battery_img(void)
{
	...
	if(battery_t.charging == POC_CHG_DISCONNECTED)//设备未充电
	{
		lv_task_t * task = lv_task_create(lv_poc_charge_power_outages_shutdown_task,
			POC_CHARGE_POWER_DOWN_TIME, LV_TASK_PRIO_MID, NULL);
		lv_task_once(task);
	}
	else//设备在充电
	{
