...
/*
  ...
  describe : 刷新列表
	  date : 2020-06-17
*/
void lv_poc_list_repeat_refresh(poc_update_type LIST_TYPE , lv_obj_t * list)
{
	list_refresh_status = false;//刷新机制开启
	lv_task_t * task = lv_task_create(lv_poc_list_update_task, 10, LV_TASK_PRIO_LOWEST, (void *)LIST_TYPE);
	activity_list = list;
	lv_task_once(task);
}
...
static void lv_poc_list_update_task(lv_task_t * task)
{
	...
	else if(user_data_type == LVPOCUPDATE_TYPE_GROUPLIST)
	{
		LV_ASSERT_OBJ(activity_list, LV_OBJX_NAME);
