...
static lv_obj_t *label_array[10] = {};
...
static char lv_poc_state_info_text_operator[64] = {0};
...
lv_poc_state_info_label_struct_t lv_poc_state_info_label_array[] = {
	...
	{
		NULL,
		"运营商"                    , LV_LABEL_LONG_SROLL_CIRC, LV_LABEL_ALIGN_LEFT, LV_ALIGN_IN_LEFT_MID  ,
		lv_poc_state_info_text_operator, LV_LABEL_LONG_SROLL_CIRC, LV_LABEL_ALIGN_LEFT, LV_ALIGN_OUT_RIGHT_MID, 0, 0,
	},
};
...
static int lv_poc_state_info_network_server(void)
{
	...
	if(lvPocGuiCtelCom_get_status() == 3)
	{
		if(uState == 0)
		{
...
static void lv_poc_state_info_operator(void)
{
	char operator_name[24] = "";
	lv_poc_get_mobile_card_operator(operator_name,false);
	strcpy(lv_poc_state_info_text_operator, operator_name);
}
...
static void lv_poc_refresh_state_info_operator(void)
{
	lv_poc_state_info_operator();
	lv_label_set_text(label_array[9], lv_poc_state_info_label_array[9].content);
}
