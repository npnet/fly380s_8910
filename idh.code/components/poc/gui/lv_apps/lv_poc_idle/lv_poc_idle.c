...
static lv_obj_t * idle_operator = NULL;
...
static lv_res_t lv_poc_idle_signal_func(struct _lv_obj_t * obj, lv_signal_t sign, void * param)
{
	OSI_PRINTFI("[idle][sigal](%s)(%d):key(%d), sign(%d)", __func__, __LINE__, *(unsigned int *)param, sign);
	switch(sign)
	{
...
static void lv_poc_idle_time_task(void)
{
    ...
    {
        lv_obj_align(idle_big_clock, activity_idle->display, LV_ALIGN_CENTER, -1, -screen_h * 4 / 26);
    }
    else
    {
...
lv_poc_activity_t * lv_poc_create_idle(void)
{
	...
	lv_poc_control_text_t control = {"主菜单", "", ""}; //{"主菜单","群组","下一页"}
	if(activity_idle != NULL)
	{
		return NULL;
	}
