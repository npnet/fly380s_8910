...
static void prv_lv_poc_member_list_refresh_with_data(lv_poc_member_list_t *member_list_obj);
...
static void prv_lv_poc_group_list_refresh_with_data(lv_poc_group_list_t *group_list_obj);
...
uint8_t lv_poc_get_battery_cnt(OUT battery_values_t *values)
{
    ...
    else if(values->battery_value >= 4)
    {
		battery_img_cnt=1;
    }
            ...
            if(sim_state_code[k] & 0x01)
            {
                *(has_sim[k]) = true;
				...
				case 6://显示无服务
				{
					...
					{
						is_net_type = 2;
						lvPocLedCom_Msg(LVPOCLEDIDTCOM_SIGNAL_NO_NETWORK_STATUS, true);
					}
					break;
...
bool lv_poc_cbn_key_obj(lv_indev_data_t *data)
{
	...
	switch(data->key)
	{
		...
		case LV_GROUP_KEY_MB:
		{
			if(lv_poc_cit_get_run_status() == LV_POC_CIT_OPRATOR_TYPE_KEY)
			{
				lv_poc_type_key_member_cb(true);
				return false;
			}
